...
4. THEN search for: "${fullTopic} ${query} advanced" or "${fullTopic} ${query} patterns" for comprehensive usage patterns.
5. IF version is specified, search for: "${fullTopic} ${query} new features" or "${fullTopic} ${query} breaking changes".
6. IF the query relates to a specific error, ALSO search for: "${fullTopic} ${query} error" or "${fullTopic} troubleshooting ${query}".
7. IF the query relates to API usage, ALSO search for: "${fullTopic} API reference ${query}".
8. IF searching for newer frameworks/libraries, ALSO check GitHub repositories for examples in README files, examples directory, or official docs directory.
9. FINALLY search for: "${fullTopic} ${query} best practices" or "${fullTopic} ${query} performance" for optimization techniques.
...
2. CODE SNIPPET PRESENTATION:
   ...
   b. If multiple snippets are found, arrange them in order of complexity (basic → intermediate → advanced)
   c. Include essential context and explain when/why to use each pattern
   ...
   f. Include performance implications or best practice notes when available in documentation
   g. Show both minimal and comprehensive examples when documentation provides both approaches
...
4. RESPONSE STRUCTURE:
   ...
   b. Format must be:
      ...
      [Essential context explaining the pattern and when to use it]
      
      ${include_examples ? "[Additional examples showing variations, advanced usage, or related patterns]" : ""}
      
      ${version ? "[Version-specific features or changes if applicable]" : ""}
   ...
   e. For complex topics, show progression from basic to advanced usage patterns
