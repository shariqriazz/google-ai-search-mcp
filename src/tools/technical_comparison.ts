...
4. THEN search for quantitative metrics and benchmarks: "${technologies.map(t => `${t} performance benchmarks`).join('", "')}", "${technologies.map(t => `${t} bundle size`).join('", "')}"
5. THEN search for version-specific information: "${technologies.map(t => `${t} release notes`).join('", "')}"
6. THEN search for community surveys and adoption statistics: "${techString} usage statistics", "${techString} developer survey", "${techString} market share"
7. THEN search for real-world usage metrics: "${technologies.map(t => `companies using ${t}`).join('", "')}", "${techString} case studies"
8. IF a specific use case was provided, search for: "${techString} for ${use_case}"
9. FINALLY search for migration complexity: "${technologies.map(t1 => technologies.filter(t2 => t1 !== t2).map(t2 => `migrating from ${t1} to ${t2}`).join(', ')).join(', ')}"
...
2. COMPREHENSIVE COVERAGE:
   ...
   f. Include quantitative metrics wherever possible (performance benchmarks, bundle sizes, load times)
   g. Provide market adoption statistics and real-world usage data
